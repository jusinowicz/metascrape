...
def split_cardinal(cardinal_value):
	...
	try:
		if len(parts) == 2:
			# Return the response size and the standard error
			return float(parts[0]), float(parts[1])
		elif len(parts) == 1:
			# Return the response size and None for SE if there's only one value
			return float(parts[0]), None
		else:
			# If CARDINAL contains unexpected text format, return None for both
			return None, None
	except ValueError:
		# If conversion to float fails, return None for both
		return None, None